...
class ArticleController extends Controller
{
    ...
    public function list($id, Request $request)
    {
        ...
        $ColumnAll = Column::where('show', Column::COLUMN_SHOW_YES)
            ->where('list', Column::COLUMN_LIST_YES)
            ->select('id', 'pid', 'name')
            ->get();
        // 获取当前栏目信息
        $return['column'] = Column::select('id', 'pid', 'name', 'template', 'list', 'keyword', 'describes', 'created_at')->with(['ColumnProperty' => function ($q) {
        ...
        // 获取同级栏目列表
        $return['menu'] = Column::select('id', 'pid', 'name')->where('show', Column::COLUMN_SHOW_YES)->where('pid', $return['column']->pid)->orderBy('sort', 'ASC')->orderBy('created_at', 'ASC')->get();
